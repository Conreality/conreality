...
let v2_op_eq () = same_bool true (V2.eq V2.zero V2.zero)
...
let v3_create () = same_float_list [e; pi; phi] (v3_to_list tvec3_2)
...
(* Keep this one operating on floats to avoid depending on V3.eq *)
let v3_zero () = same_float_list [0.; 0.; 0.] (v3_to_list tvec3_0)
...
let p2_create () = same_float_list [e; pi] (p2_to_list tp2_2)
let p2_x () = same_float 3. (P2.x tp2_1)
let p2_y () = same_float pi (P2.y tp2_2)
let p2_el () = same_float 3. (P2.el tp2_1 0)
(* Keep this one operating on floats to avoid depending on P2.eq *)
let p2_zero () = same_float_list [0.; 0.] (p2_to_list tp2_zero)
let p2_eq () = same_bool true (P2.eq P2.zero P2.zero)
let p2_op_eq () = same_bool true (P2.( = ) P2.zero P2.zero)
let p2_mid () = same_bool true (P2.eq (P2.mid tp2_1 tp2_zero) (P2.create (1.5) (0.5)))
...
let p3_create () = same_float_list [e; pi; phi] (p3_to_list tp3_2)
...
(* Keep this one operating on floats to avoid depending on P3.eq *)
let p3_zero () = same_float_list [0.; 0.; 0.] (p3_to_list tp3_zero)
let p3_eq () = same_bool true (P3.eq P3.zero P3.zero)
let p3_op_eq () = same_bool true (P3.( = )P3.zero P3.zero)
...
(* Keep this one operating on floats to avoid depending on P3.eq *)
let m2_create () = same_float_list [e; pi; phi; 0.] (m2_to_list tm2_2)
...
(* Keep this one operating on floats to avoid depending on P3.eq *)
let m2_zero () = same_float_list [0.; 0.; 0.; 0.] (m2_to_list tm2_zero)
let m2_id () = same_float_list [1.; 0.; 0.; 1.] (m2_to_list M2.id)
let m2_el () = same_float 3.14159 (M2.el 0 1 tm2_2)
let m2_neg () = same_bool true (M2.eq (M2.neg tm2_1) (M2.create (-3.) (-1.) (-2.) (-7.)))
let m2_add () = same_bool true (M2.eq (M2.create (6.) (2.) (4.) (14.)) (M2.add tm2_1 tm2_1))
let m2_op_add () = same_bool true (M2.eq (M2.create (6.) (2.) (4.) (14.)) (M2.(+) tm2_1 tm2_1))
let m2_sub () = same_bool true (M2.eq (M2.sub tm2_1 tm2_1) tm2_zero)
let m2_op_sub () = same_bool true (M2.eq (M2.( - ) tm2_1 tm2_1) tm2_zero)
let m2_eq () = same_bool true (M2.eq M2.zero M2.zero)
let m2_op_eq () = same_bool true (M2.( = ) M2.zero M2.zero)
let m2_smul () = todo ()
