...
%{
  ...
  let degrees_to_radians deg =
    ...
    else Exception.semantic_error (sprintf "degrees must be in range 0 to 360 (got %f)" deg)

  let clock_to_radians pos =
    if pos >= 0. && pos <= 12.
    then degrees_to_radians (360. /. 12.) *. (mod_float pos 12.)
    else Exception.semantic_error (sprintf "clock positions must be in range 0 to 12 (got %f)" pos)
%}
